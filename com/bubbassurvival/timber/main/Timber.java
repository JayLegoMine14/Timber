...
public class Timber extends JavaPlugin {
        ...
        this.getServer().getPluginManager().registerEvents((Listener) new Listener(){
            ...
            public void timberIt(final BlockBreakEvent event){
                ...
                if(isLog(event.getBlock()) && Timber.this.getConfig().getIntegerList("timber-items").contains(ih)){
                    ...
                    if(event.getPlayer().getGameMode() == GameMode.SURVIVAL && looseDurability(item)){
    ...
    private boolean looseDurability(ItemStack i){
        if(i.containsEnchantment(Enchantment.DURABILITY)){
            double level = i.getEnchantmentLevel(Enchantment.DURABILITY);
            if(Math.random() <= (100 / (level + 1)) / 100) return true;
        }else
            return true;
